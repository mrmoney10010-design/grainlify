...
use crate::{
    test_batch_operations::setup,
    test_batch_operations::init_program,
    test_batch_operations::Ctx,
...
fn test_program_archival_success() {
    ...
    let info = ctx.client.get_program_info_v2(&String::from_str(&ctx.env, program_id));
    assert_eq!(info.total_funds, 1000);
    assert!(!info.archived);
    assert_eq!(info.archived_at, None);

    // Archive program
    ctx.client.archive_program(&String::from_str(&ctx.env, program_id));
    ...
    assert_eq!(archived.get(0).unwrap(), String::from_str(&ctx.env, program_id));
}
...
fn test_program_archival_filtering() {
    ...
    ctx.client.archive_program(&String::from_str(&ctx.env, "SINGLETON"));
...
fn test_archive_non_existent_program() {
    let ctx = setup();
    ctx.client.archive_program(&String::from_str(&ctx.env, "NON_EXISTENT"));
}
